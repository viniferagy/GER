...
class RepReadingPipeline(Pipeline):
    ...
    def _batched_string_to_hiddens_all(self, train_inputs, rep_tokens, hidden_layers, batch_size, which_hidden_states, **tokenizer_args):
        # Like _batched_string_to_hiddens, but each input may request multiple token positions.
        if not isinstance(hidden_layers, list):
            hidden_layers = [hidden_layers]
        assert len(train_inputs) == len(rep_tokens), "rep_tokens must have one token-index list for each input"

        hidden_states = {layer: [] for layer in hidden_layers}
        tokenizer_args = dict(tokenizer_args)
        tokenizer_args.setdefault("padding", True)

        batch_starts = range(0, len(train_inputs), batch_size)
        disable_progress = os.environ.get("GER_REPE_DISABLE_PROGRESS", "").strip() in {
            "1", "true", "True", "TRUE", "yes", "Yes", "YES"
        }
        iterator = tqdm(
            batch_starts,
            total=(len(train_inputs) + batch_size - 1) // batch_size,
            desc=os.environ.get("GER_REPE_PROGRESS_DESC", "RepE hidden states"),
            unit="batch",
        ...
        for start in iterator:
            ...
            batch_rep_tokens = rep_tokens[start:end]

            model_inputs = self.tokenizer(batch_inputs, return_tensors=self.framework, **tokenizer_args)
            model_inputs = self._ensure_tensor_on_device(model_inputs, device=self.device)

            with torch.no_grad():
                if hasattr(self.model, "encoder") and hasattr(self.model, "decoder"):
                    decoder_start_token = [self.tokenizer.pad_token] * model_inputs["input_ids"].size(0)
                    decoder_input = self.tokenizer(decoder_start_token, return_tensors=self.framework).input_ids
                    model_inputs["decoder_input_ids"] = decoder_input.to(model_inputs["input_ids"].device)
                outputs = self.model(**model_inputs, output_hidden_states=True)
            ...
            for layer in hidden_layers:
                layer_hidden_states = outputs["hidden_states"][layer].detach()
                if layer_hidden_states.dtype == torch.bfloat16:
                    layer_hidden_states = layer_hidden_states.float()

                for item_idx, item_rep_tokens in enumerate(batch_rep_tokens):
                    item_hiddens = []
                    for rep_token in item_rep_tokens:
                        token_idx = rep_token if rep_token >= 0 else seq_len + rep_token
                        if token_idx < 0 or token_idx >= seq_len:
                            # Token alignment can land exactly on the truncated boundary for
                            # long GEC prompts. Preserve the pos/neg pairing by using the
                            # nearest valid token instead of dropping one side of the pair.
                            token_idx = min(max(token_idx, 0), seq_len - 1)
                        item_hiddens.append(layer_hidden_states[item_idx, token_idx, :].cpu().numpy())
                    hidden_states[layer].append(np.vstack(item_hiddens))
