...
# The YAML files may now contain only semantic experiment settings; machine paths,
# model locations, and generated result dirs can come from the task runner.

PIPELINE_COMMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MULTILINGUAL_DIR_FROM_PIPELINE="$(cd "${PIPELINE_COMMON_DIR}/../.." && pwd)"
PROJECT_ROOT_FROM_PIPELINE="$(cd "${MULTILINGUAL_DIR_FROM_PIPELINE}/.." && pwd)"
...
resolve_project_model_path() {
    ...
    case "$original" in
        ...
        *Qwen2.5-7B-Instruct)
            ...
            ;;
        Qwen/Qwen2.5-14B-Instruct|*Qwen2.5-14B-Instruct)
            echo "${GER_QWEN14B_MODEL_PATH:-${model_root}/Qwen2.5-14B-Instruct}"
            ;;
        *xlm-roberta-large)
            echo "${GER_XLM_ROBERTA_MODEL_PATH:-${model_root}/xlm-roberta-large}"
            ;;
        *deepseek)
            echo "${GER_DEEPSEEK_MODEL_PATH:-${model_root}/deepseek}"
            ;;
        *Llama-3-8b-Instruct)
            echo "${GER_LLAMA3_8B_MODEL_PATH:-${model_root}/Llama-3-8b-Instruct}"
            ;;
        *gte-Qwen2-1.5B-instruct)
            echo "${GER_GTE_QWEN_MODEL_PATH:-${model_root}/gte-Qwen2-1.5B-instruct}"
            ;;
        *)
            echo "$original"
            ;;
...
apply_icl_runtime_defaults() {
    ...
    if [ -z "${DATASET:-}" ] && [ -n "${DATASET_NAME:-}" ]; then
        DATASET="${DATASET_NAME}:${DATABASE_NAME}"
    fi
